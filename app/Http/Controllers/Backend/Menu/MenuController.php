...
class MenuController extends Controller
{
    ...
    public function listMenu(Request $request)
    {
        ...
        $menu = $this->filterLkhMenusForUser($menu, $request->user());

        return response()->json(['menu' => $menu])->header('Content-Type', 'application/json');
    }

    /**
     * Sembunyikan menu LKH tertentu jika user tidak punya akses modul.
     */
    protected function filterLkhMenusForUser(Collection $menu, User $user): Collection
    {
        $hidePengajuan = ! $user->canAccessLkhPengajuan();
        $hideMonitoring = ! $user->canAccessLkhMonitoring();

        return $menu
            ->filter(function ($item) use ($hidePengajuan, $hideMonitoring) {
                if ($hidePengajuan && $item->code === 'lkh-pengajuan') {
                    return false;
                }
                if ($hideMonitoring && $item->code === 'lkh-monitoring') {
                    return false;
                }
            ...
            ->map(function ($item) use ($user) {
                if ($item->relationLoaded('accessChildren') && $item->accessChildren->isNotEmpty()) {
                    $item->setRelation(
                        'accessChildren',
                        $this->filterLkhMenusForUser($item->accessChildren, $user)
                    );
                }
